...
# Define constants
hbar = 5308
w_c = 120 / hbar # Can be modified according to your needs
beta = 0.01869 # Can be modified according to your needs
coupling_str = 1.0
N = 1 # Number of bases
init_guass = 1
...
def integrand_real(w, t):
    return w * np.exp(-w / w_c) * np.cos(w * t) / np.tanh(0.5 * beta * hbar * w)

def integrand_imag(w, t):
    return w * np.exp(-w / w_c) * np.sin(w * t)


# Define the original function (integral result)
...
# Define the fitting function using multiple bases
...
initial_guess_real = [init_guass, init_guass, init_guass] * N  # Initial guess for parameters
popt_real, pcov_real = curve_fit(fit_function_real, t_values, y_values_real, p0=initial_guess_real, maxfev=10000000, bounds=bounds)
...
def fit_function_imag(t, *params):
    ...
    for i in range(N):
        ...
        a = params[i]
        w = w_values[i]
        g = g_values[i]  # Use previously fitted g value
        '''
        a = params[3 * i]
        g = params[3 * i + 1]
        w = params[3 * i + 2]
        '''
        result += a * np.sin(w * t) * np.exp(-g * t)
    return result

# Generate sample data
y_values_imag = np.array([original_function_imag(t) for t in t_values])

#initial_guess_imag = [0.0001, 0.0001, 0.0001] * N 
initial_guess_imag = [init_guass] * N 
popt_imag, pcov_imag = curve_fit(fit_function_imag, t_values, y_values_imag, p0=initial_guess_imag, maxfev=10000000, bounds=bounds)
...
if bathtype_index is not None:
    # Overwrite 'artificial' after the 'BATHTYPE' line if something exists
    lines[bathtype_index + 1] = "artificial\n"
else:
...
if bath_index is not None:
    ...
    bath_data = []
    bath_data.append(f"{w_c * hbar:.1f} {coupling_str:.1f}\n")
...
# Define a function to find the exact match of a keyword in the file
def find_keyword_index(keyword, lines):
    pattern = rf'^{keyword}\b'  # Match the keyword as a whole word at the beginning of a line
    for i, line in enumerate(lines):
        if re.search(pattern, line):
            return i
    return None
...
bathtype_index = find_keyword_index('BATHTYPE', lines)
if bathtype_index is not None:
    # Insert 'artificial' after the 'BATHTYPE' line
    lines.insert(bathtype_index + 1, "artificial\n")
else:
    print("Keyword 'BATHTYPE' not found in the file.")
...
if bath_index is not None:
    ...
    # Insert all bath_data after the 'BATH' line
    lines[bath_index + 1:bath_index + 1] = bath_data
else:
    print("Keyword 'BATH' not found in the file.")
